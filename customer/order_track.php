<?php
// customer/order_track.php
$page_title = "Track Your Order | QuickShip";
...
if ($selected_order_id) {
    try {
        ...
        ");
        $stmt->execute([$selected_order_id, $customer_id]);
        $order = $stmt->fetch();
        ...
        if ($order) {
            ...
            $error_msg = "Order #" . $selected_order_id . " not found or you do not have permission to view it.";
...
// Fetch all orders for this customer if no single order selected
$orders = [];
if (!$selected_order_id) {
    ...
    try {
        if (!empty($search)) {
            $stmtAll = $pdo->prepare("
                ...
                FROM `orders` o 
                JOIN `delivery_partners` p ON o.delivery_partner_id = p.id
                WHERE o.customer_id = ? AND (o.id = ? OR p.name LIKE ? OR o.status LIKE ?)
                ORDER BY o.id DESC
            ");
            $stmtAll->execute([$customer_id, $search, "%$search%", "%$search%"]);
        } else {
            $stmtAll = $pdo->prepare("
                SELECT o.*, p.name AS partner_name 
                FROM `orders` o 
...
<div class="row mb-4">
    <div class="col">
        <h2 class="h3 mb-0 text-gray-800">
            <i class="fa-solid fa-route text-success me-2"></i>
            <?php echo $selected_order_id ? "Tracking Order #$selected_order_id" : "Track Shipments"; ?>
        </h2>
        <p class="text-muted">
            <?php echo $selected_order_id ? "Real-time visual map and delivery tracking checkpoints." : "View progress and delivery milestones of all your orders."; ?>
        </p>
    </div>
</div>
...
<?php if ($selected_order_id && $order): ?>
    <!-- Mode 1: Detailed Timeline Visualizer -->
    <div class="row">
        <!-- Order details block -->
        <div class="col-lg-5 mb-4">
            <div class="card main-card mb-4 h-100">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 text-success"><i class="fa-solid fa-cube me-2"></i>Shipment Info</h5>
                    <span class="badge bg-success font-monospace">Order #<?php echo $order['id']; ?></span>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label class="form-label form-label-custom text-muted mb-0">Delivery Destination</label>
                    ...
                    <div class="mb-3">
                        <label class="form-label form-label-custom text-muted mb-0">Assigned Driver Agent</label>
                        <div class="fw-bold text-dark"><i class="fa-solid fa-user-tag me-1 text-success"></i><?php echo htmlspecialchars($order['partner_name']); ?></div>
                    ...
                    <div class="mb-3">
                        <label class="form-label form-label-custom text-muted mb-0">Package Specifications</label>
                    ...
                    <div class="mt-4">
                        <a href="order_track.php" class="btn btn-custom btn-outline-secondary w-100">
                            <i class="fa-solid fa-arrow-left me-1"></i> Back to All Shipments
                        </a>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-7 mb-4">
            <div class="card main-card h-100">
                <div class="card-header">
                    <h5 class="mb-0 text-success"><i class="fa-solid fa-spinner me-2 animate-spin"></i>Tracking Timeline</h5>
                </div>
                <div class="card-body">
                            ...
                            <li class="timeline-item <?php echo $status_class; ?>">
                                ...
                                <div class="timeline-content">
                                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                                        ...
                                        <?php if ($is_active): ?>
                                            <span class="badge bg-warning text-dark font-monospace text-uppercase" style="font-size: 0.65rem;">Courier Updating...</span>
                                        <?php elseif ($is_completed): ?>
                                            <span class="badge bg-success font-monospace text-uppercase" style="font-size: 0.65rem;">Cleared</span>
                                        <?php endif; ?>
                                    </div>
                                    <?php if ($timestamp): ?>
                                    ...
                                    <?php else: ?>
                                        <div class="timeline-time text-muted mt-1">Pending courier...</div>
                                    <?php endif; ?>
                                </div>
                            </li>
                        <?php endforeach; ?>
...
<?php else: ?>
    <!-- Mode 2: Browse and Search Customer's Orders -->
    <div class="card main-card">
        <div class="card-header d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
            <h5 class="mb-0 text-success"><i class="fa-solid fa-table-list me-2"></i>Registered Deliveries</h5>
            ...
            <form action="order_track.php" method="GET" class="w-100 w-sm-auto">
                <div class="input-group">
                    <input type="text" name="search" class="form-control form-control-custom py-1" 
                           placeholder="Search ID, driver, or status..." value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>">
        ...
        <div class="card-body">
            ...
            <?php else: ?>
                <div class="text-center py-5">
                    ...
                    <p class="text-muted mb-0">No active shipments found.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
